...
class GumbelLSTMGenerator(LSTMGenerator):
    """A GumbelLSTMGenerator class is the one in charge of Gumbel-based Long Short-Term Memory implementation.
    ...
    def call(self, x):
        ...
        # Regularizing the output with the temperature parameter
        x /= self.tau
...
class GSGAN(Adversarial):
    ...
    def __init__(self, encoder=None, vocab_size=1, max_length=1, embedding_size=32, hidden_size=64, n_filters=[64], filters_size=[1], dropout_rate=0.25, temperature=1):
        ...
        D = LSTMGenerator(encoder, vocab_size, embedding_size, hidden_size)
    ...
    def step(self, x, y):
        ...
        z = tf.random.normal([x.shape[0], x.shape[1]])
        ...
        with tf.GradientTape() as G_tape, tf.GradientTape() as D_tape:
            ...
            x_fake = tf.argmax(x_fake, axis=-1)
